...
function split = treeSplitting(X,Y,rndFeat)
	...
	[n, nbFeatures] = size(X);
	indices = randperm(nbFeatures);
	X = X(:,indices(1:rndFeat));

	% nbAEnlever = nbFeatures - rndFeat;
	% indices = 1:nbFeatures;
	% for i = 1:nbAEnlever
	% 	indiceAEnlever = randi(nbFeatures);
	% 	X(:,indiceAEnlever) = [];
	% 	indices(indiceAEnlever) = [];
	% 	[n, nbFeatures] = size(X);
	% end
